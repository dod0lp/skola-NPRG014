...
object JsonSerializer:
  ...
  given listSerializer[T](using js: JsonSerializer[T]): JsonSerializer[List[T]] with
    def serialize(xs: List[T]) =
      xs.map(_.toJson).mkString("[ ", ", ", " ]")

  given mapSerializer[T](using js: JsonSerializer[T]): JsonSerializer[Map[String, T]] with
    def serialize(m: Map[String, T]) =
      m.map((k, v) => "\"" + k + "\": " + v.toJson).mkString("{ ", ", ", " }")

  given phoneSerializer: JsonSerializer[PhoneNo] with
  ...
  given personSerializer(using JsonSerializer[PhoneNo]): JsonSerializer[Person] with
  ...
  given addressSerializer(using JsonSerializer[Person]): JsonSerializer[Address] with
...
object JsonSerializerTest:
  def main(args: Array[String]): Unit =
    ...
    println(a1.toJson.equals("\"Hello\""))
